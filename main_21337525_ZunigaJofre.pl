...
% Caso 1: Si queda una interaccion por hacer se hace "manual" para salir
% del loop
systemSimulate(System,MaxInteractions,Seed,NewSystem):-
    ...
    systemTalkRec(System,OptionMessage,NewSystem).

% Caso 2: Si el sistema no está simulando se "resetea", deslogeando, se
% agrega un userSeed y se conversa con systemTalkRec y se establece un 1
% en systemSimulateOn para notar que el sistema está simulando
systemSimulate(System,MaxInteractions,Seed,NewSystem):-
...
% Caso 3: Si ya se inicio una simulación se genera un nuevo numero
